...
def generate_dates(mode, year, min_c, max_c, month=None, weekday=None, skip_pct=15, force_full=False):
    ...
    while current <= end:
        if mode == "full":
            add_day(current)
        elif mode == "month" and current.month == month:
            if random.randint(1, 100) > skip_pct:
                add_day(current)
        elif mode == "day" and current.weekday() == weekday:
            add_day(current)
        ...
        if action == "1":
            ...
            if mode_choice == "1":
                ...
                for k, v in MONTHS.items():
                    menu_item(k, v)
                print()
                month = int(prompt("Month number (1-12)"))
            elif mode_choice == "4":
                mode = "day"
                ...
                for k, v in DAYS.items():
                    menu_item(k, v)
                print()
                weekday = int(prompt("Weekday number (0=Monday, 6=Sunday)"))
            else:
                error("Invalid mode.")
                continue
            ...
            info(f"Year       : {year}")
            info(f"Mode       : {mode}" + (" [FORCE FULL YEAR]" if force_full else "") + (f" / {MONTHS.get(month,'')}" if month else "") + (f" / {DAYS.get(weekday,'')}" if weekday is not None else ""))
            ...
            dates = generate_dates(mode, year, min_c, max_c, month, weekday, skip_pct, force_full=force_full)
